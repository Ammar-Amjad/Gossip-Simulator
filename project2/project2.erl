...
%% API
-export([run/3, actorGossip/2, printList/1, printelement/3, gossip2D/1, gossipFull/1, getidx2D/4, sendRumor2DGossip/5,
...
sendRumor2DGossip(Rumor, Left, Right, Up, Down) ->
  Turn = rand:uniform(4),
  if
    (Left > -1) and (Turn == 1) -> Left ! {rumor, Rumor};
    (Right > -1) and (Turn == 2) -> Right ! {rumor, Rumor};
    (Up > -1) and (Turn == 3) -> Up ! {rumor, Rumor};
    (Down > -1) and (Turn == 4) -> Down ! {rumor, Rumor};
    true -> done
  end,
  sendRumor2DGossip(Rumor, Left, Right, Up, Down).
...
sendRumorFullGossip(Rumor, ListofList, Rowcount) ->
  ...
  lists:nth(J, lists:nth(I, ListofList)) ! {rumor, Rumor},
  sendRumorFullGossip(Rumor, ListofList, Rowcount).
...
gossip2D(NumNodes) ->
  ...
  ListofList = [[spawn(project2, actorGossip, [self(), Rowcount]) || _ <- lists:seq(1, Rowcount)] || _ <- lists:seq(1, Rowcount)],
  % printList(ListofList),
...
gossipLine(NumNodes) ->
  % 2D grid
  Rowcount =  round(math:sqrt(NumNodes)),
  List = [spawn(project2, actorGossip, [self(), Rowcount]) || _ <- lists:seq(1, Rowcount*Rowcount)],
  % printList(List),
...
pushSum2D(NumNodes) ->
  Rowcount =  round(math:sqrt(NumNodes)),
  ListofList = [[spawn(project2, actorPushSum, [self(), Rowcount]) || _ <- lists:seq(1, Rowcount)] || _ <- lists:seq(1, Rowcount)],
  % printList(ListofList),
...
pushSumLine(NumNodes) ->
  Rowcount =  round(math:sqrt(NumNodes)),
  List = [spawn(project2, actorPushSum, [self(), Rowcount]) || _ <- lists:seq(1, Rowcount*Rowcount)],
  % printList(List),
...
run(NumNodes, Topology, Algorithm) ->
  case Algorithm of
    "gossip" ->
      case Topology of
        "2D" -> gossip2D(NumNodes);
        "full" -> gossipFull(NumNodes);
        "Imp3D" -> gossipI2D(NumNodes);
        "line" -> gossipLine(NumNodes)
      end;
    "push-sum" ->
      case Topology of
